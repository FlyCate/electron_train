...
void CxSkinButton::SetSkin(CString normal,CString down,CString over,CString disabled, CString focus,CString mask,
	short drawmode, short border, short margin)
{
	m_bNormal.DeleteObject();	//free previous allocated bitmap
	m_bDown.DeleteObject();
	...
	if (normal !="") 
	\
	   m_bNormal.Attach((HBITMAP)LoadImage(AfxGetInstanceHandle(),normal,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION));
	if (down !="") 
		m_bDown.Attach((HBITMAP)LoadImage(AfxGetInstanceHandle(),down,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION));
	if (over !="") 
		m_bOver.Attach((HBITMAP)LoadImage(AfxGetInstanceHandle(),over,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION));
	if (focus !="") 
		m_bFocus .Attach( (HBITMAP)LoadImage(AfxGetInstanceHandle(),focus,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION));
	if (disabled !="") 
		m_bDisabled .Attach( (HBITMAP)LoadImage(AfxGetInstanceHandle(),disabled,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION));
	else if (normal !="") 
		m_bDisabled .Attach( (HBITMAP)LoadImage(AfxGetInstanceHandle(),normal,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION));
	m_DrawMode=max(0,min(drawmode,2));
	m_Border=border;
	m_FocusRectMargin=max(0,margin);
	if (mask !="") 
	{
		m_bMask .Attach(  (HBITMAP)LoadImage(AfxGetInstanceHandle(),mask,IMAGE_BITMAP,0,0,LR_LOADFROMFILE|LR_CREATEDIBSECTION));
		if (hClipRgn) DeleteObject(hClipRgn);
		hClipRgn = CreateRgnFromBitmap(m_bMask,RGB(255,255,255));
		if (hClipRgn){
			SetWindowRgn(hClipRgn, TRUE);
